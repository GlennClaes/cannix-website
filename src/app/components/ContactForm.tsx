...
export function ContactForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" noValidate>
      ...
      {status === "success" && (
        <div className="flex items-start gap-3 p-4 rounded-xl bg-green-500/10 border border-green-500/30 text-green-300" role="status">
      ...
      {status === "error" && (
        <div className="flex items-start gap-3 p-4 rounded-xl bg-red-500/10 border border-red-500/30 text-red-300" role="alert">
          <AlertCircle className="h-5 w-5 flex-shrink-0 mt-0.5" />
          <div>
            <p className="font-semibold">Versturen lukte niet.</p>
            <p className="text-sm text-red-300/80">Probeer opnieuw of mail direct naar bookings@cannix.be.</p>
          </div>
        </div>
      )}
